...
int main(void)
{
  ...
  SystemClock_Config();
	...
	/* FSI => the below code is a trap : if programm flashed on another chip, system restart endlessly
    if( strncmp(STM32_UUID, (const char*)au8_lock, 12 ) != 0 )
    {
       NVIC_SystemReset(); //<= to be patched???
    }
	*/ 
    ...
    //MX_IWDG_Init(); //FSI => Init watchdog not used (yet)

		//FSI => two lines below are for input config , legagy from LEAF battery swap, could be usefull in the future
    ...
    while (1)
    {           
        //HAL_IWDG_Refresh(&hiwdg); // FSI => take care of watchdog , not used (yet)
			
				// FSI => below three lines are for copy/paste wherever you want to generate state for led (very slow!)
        ...
        if(( HAL_GetTick() - last_tick ) >= 1000u )
        {
            ...
            //one_second_ping(); //FSI not usefull here legacy from Dala
        ...
        { 
            ...
            can_imiev_handler( MYCAN1, &frame ); //manage frame alteration if necessary
        }
        ...
        {   
            ...
            can_imiev_handler( MYCAN2, &frame );            
        }
